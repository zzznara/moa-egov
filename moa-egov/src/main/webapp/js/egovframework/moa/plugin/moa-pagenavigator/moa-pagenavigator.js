...
*/
    var page = $("[name='searchVo.pageIndex']").val();
...
*/
      if (num == 1) {
          jQuery('a', target).attr('href', "javascript:doSearch("+ page +")");
      } else if (num == 2) {
          jQuery(target).attr('href', "javascript:doSearch("+ page +")");
      } else {}
...
var MoaPagenavi = {
		...
		setLayer : function()
		{
				...
				var pageObjLength = $(".pagination").length;
			
				if( pageObj != null )
				{
					for( var i = 0; i < pageObjLength; i++ )
					{
							// 초기의 빈 리스트가 사라지므로 전역변수에 담아놓는다.
							if( !MoaPagenavi._layout[i] ) {
								MoaPagenavi._layout[i] = pageObj.clone();
							}
		...
		/* 페이지 네비게이션 */
		view : function( totalCount, pageRow, pageLink )
		{
				if( pageRow == null || pageRow == 0 ) pageRow = 10;
				if( pageLink == null || pageLink == 0 ) pageLink = 10;
				...
				for( var i = 0; i < pageObjLength; i++ )
				{
						pageObj.eq(i).empty().append( MoaPagenavi._layout[i].html() );
						
						pageObj.eq(i).jaPageNavigator({
						      page_row : pageRow				// 보여질 게시물 목록 수
						    , page_link : pageLink				// 보여질 페이지 링크 수
						    , total_count : totalCount			// 게시물 총 수
						});
				}
		}
}
